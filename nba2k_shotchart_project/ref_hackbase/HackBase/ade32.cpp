...
int disasm(BYTE* opcode0, disasmstruct* diza){
	...
	memset(diza, 0x00, sizeof(disasmstruct));
	diza->disasmdefdata = 4;
	diza->disasmdefaddr = 4;
...
repeat_prefix:
	...
	if(t & C_ANYPREFIX){
		...
		if(t & C_67)
			diza->disasmdefaddr ^= 2^4;
		else if(t & C_66)
			diza->disasmdefdata ^= 2^4;
		else if(t & C_SEG)
			diza->disasmseg = c;
		else if(t & C_REP)
			diza->disasmrep = c;
	...
	if(flag & C_MODRM){
		...
		if(((c & 0x38) == 0x20) && (diza->disasmopcode == 0xFF))
			flag |= C_STOP;
	...
	diza->disasmflag = flag;

	DWORD a =  flag & (C_ADDR1 | C_ADDR2 | C_ADDR4);
	DWORD d = (flag & (C_DATA1 | C_DATA2 | C_DATA4)) >> 8;

	if(flag & C_ADDR67)
		a += diza->disasmdefaddr;
	if(flag & C_DATA66)
		d += diza->disasmdefdata;

	diza->disasmaddrsize = a;
	diza->disasmdatasize = d;

	DWORD i;
	for(i = 0; i < a; i++)
		diza->disasmaddr_b[i] = *opcode++;

	for(i = 0; i < d; i++)
		diza->disasmdata_b[i] = *opcode++;

	diza->disasmlen = opcode - opcode0;

	return diza->disasmlen;
} // disasm()
...
int oplen(BYTE *opcode){
	...
	if((diza.disasmflag == C_ERROR) || ((diza.disasmflag&C_STOP) == C_STOP) || ((diza.disasmflag&C_REL)==C_REL) || ((diza.disasmflag&C_BAD)==C_BAD))
		return -1;
