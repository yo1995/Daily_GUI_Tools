...
void init_main() {
	...
	if (!mHackBase->Initialize(onRender, exename, windowname))
		MessageBox(0, "Error hooking game. Maybe injected into wrong process...", "Failed to hook...", 0);
	...
	*/
	while (true) { //infinite loop!
		if (GetAsyncKeyState(VK_DOWN)) {
			MessageBox(0, "We are down.", "key pressed", 0);
		}
		if (GetAsyncKeyState(VK_UP)) {
			MessageBox(0, "We are up.", "key pressed", 0);
		}
